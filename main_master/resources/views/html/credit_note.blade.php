         ...
         <div class="pcs-template-bodysection">
                  ...
                  <tbody>
                     <tr>
                        <td style="padding-bottom: 10px;">
                           <span class="pcs-label">Credit Note No</span>
                           <span style="font-weight: 600;">: {{ $credit_note->credit_note_number }}</span>
                           <span class="pcs-label">Credit Date</span>
                           <span style="font-weight: 600;">:
                              {{ date('d/m/Y', strtotime($credit_note->credit_note_date)) }}</span>
                           <span class="pcs-label">Invoice#</span>
                           <span style="font-weight: 600;">: {{ model($credit_note->invoice, 'invoice_number') }}</span>
                           <span class="pcs-label">Invoice Date </span>
                           <span style="font-weight: 600;">:
                              {{ date('d/m/Y', strtotime(model($credit_note->invoice, 'invoice_number'))) }}</span>
                        </td>
                     </tr>
                  </tbody>
               </table>
                  ...
                  <tr>
                     <td style="padding-bottom: 10px;" valign="top">
                        <span style="white-space: pre-wrap;line-height: 15px;"><strong><span
                                 class="pcs-customer-name"><a href="#">{{ model($credit_note->customer, 'firstname') }}
                                    {{ model($credit_note->customer, 'lastname') }}</a></span></strong><br>
                           <b>{{  $credit_note->organisation->org_street1 }}</b>
                           <br>{{  $credit_note->organisation->org_street2 }}<br>{{ $credit_note->organisation->org_city }}
                           <br>{{ $credit_note->organisation->org_postal }}
                           {{ $credit_note->organisation->org_state }}<br>{{ $credit_note->organisation->country }}</span>
                     </td>
                  </tr>
               </tbody>
            </table>
            ...
            <table style="width: 100%;table-layout:fixed;clear: both;" class="pcs-itemtable" cellspacing="0"
               ...
               <tbody class="itemBody">
                  @include('html.itemDetail', ['item_details' => $credit_note->creditNoteDetails])
               </tbody>
            </table>
            <div style="width: 100%;">
                     ...
                     <tbody>
                        <tr>
                           ...
                           <td valign="middle" style="width:110px;">
                              {{ number_format(array_sum($credit_note->creditNoteDetails->pluck('item_gross')->toArray()), 2) }}
                           </td>
                        </tr>
                        <tr>
                           <td valign="middle">Vat</td>
                           <td valign="middle" style="width:110px;">
                              {{ number_format(array_sum($credit_note->creditNoteDetails->pluck('item_vat')->toArray()), 2) }}
                           </td>
                        </tr>
                        <tr style="height:10px;">
                           <td valign="middle">Excise</td>
                           <td valign="middle" style="width:110px;">
                              {{ number_format(array_sum($credit_note->creditNoteDetails->pluck('item_net')->toArray()), 2) }}
                           </td>
                        </tr>
                        <tr style="height:10px;">
                           <td valign="middle">Net Total</td>
                           <td valign="middle" style="width:110px;">
                              {{ number_format(array_sum($credit_note->creditNoteDetails->pluck('item_excise')->toArray()), 2) }}
                           </td>
                        </tr>
                        <tr style="height:10px;">
                           <td valign="middle">Discount</td>
                           <td valign="middle" style="width:110px;">
                              {{ number_format(array_sum($credit_note->creditNoteDetails->pluck('item_discount_amount')->toArray()), 2) }}
                           </td>
                        </tr>
                        <tr style="height:10px; font-size:16px;" class="pcs-balance">
                           <td valign="middle"><b>Total</b></td>
                           <td valign="middle" style="width:110px;;">
                              <strong>{{ number_format(array_sum($credit_note->creditNoteDetails->pluck('item_grand_total')->toArray()), 2) }}</strong>
                           </td>
                        </tr>
                     </tbody>
                  </table>
                        ...
                        <tbody>
                           <tr>
                              ...
                              <td class="total-in-words-value text-align-right">
                                 <i><b>{{ convertToCurrency(array_sum($credit_note->creditNoteDetails->pluck('item_grand_total')->toArray())) }}</b></i>
                              </td>
                           </tr>
                        </tbody>
                     </table>
