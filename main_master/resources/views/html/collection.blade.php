            ...
            <div class="pcs-template-bodysection">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td style="width:40%;padding: 5px;vertical-align: bottom;" align="right">
                                <div class="pcs-entity-title">COLLECTION</div>
                            </td>
                        </tr>
                    </tbody>
                </table>
                        ...
                        <tbody>
                            <tr>
                                <td style="padding-bottom: 10px;">
                                    <span class="pcs-label">Collection No</span>
                                    <span style="font-weight: 600;">: {{ $collection->collection_number }}</span>
                                    <span class="pcs-label">Payment Type</span>
                                    <span style="font-weight: 600;">:
                                        @if ($collection->payemnt_type == 1)
                                            Cash
                                        @elseif ($collection->payemnt_type == 2)
                                            Cheque
                                        @else
                                            NEFT
                                        @endif
                                    ...
                                    <span style="font-weight: 600;">:
                                        {{ model($collection->invoice, 'invoice_number') }}</span>
                                    <span class="pcs-label">Invoice Date </span>
                                    <span style="font-weight: 600;">:
                                        {{ date('d/m/Y', strtotime(model($collection->invoice, 'invoice_date'))) }}</span>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                        ...
                        <tr>
                            <td style="padding-bottom: 10px;" valign="top">
                                <span style="white-space: pre-wrap;line-height: 15px;"><strong><span
                                            class="pcs-customer-name"><a
                                                href="#">{{ model($collection->customer, 'firstname') }}
                                                {{ model($collection->customer, 'lastname') }}</a></span></strong><br>
                                    <b>{{  $collection->organisation->org_street1 }}</b>
                                    <br>{{  $collection->organisation->org_street2 }}<br>{{ $collection->organisation->org_city }}
                                    <br>{{ $collection->organisation->org_postal }}
                                    {{ $collection->organisation->org_state }}<br>{{ $collection->organisation->country }}</span>
                            </td>
                        </tr>
                    </tbody>
                </table>
                ...
                <table style="width: 100%;table-layout:fixed;clear: both;" class="pcs-itemtable" cellspacing="0"
                    ...
                    <tbody class="itemBody">
                        @include('html.itemDetail', ['item_details' => $collection->collectiondetails])
                    </tbody>
                </table>
                <div style="width: 100%;">
